...
class VideoTranscriberX:
    def __init__(self, model_size="large-v2", hf_token=None):
        self.device = "cpu"
        print(f"Using device: {self.device}", flush=Trueimport os
import json
import tempfile
import shutil
from pathlib import Path
...
class VideoTranscriberX:
    def __init__(self, model_size="large-v2", hf_token=None):
        ...
        # Load pyannote diarization pipeline if token provided
        if hf_token:
            try:
...
def process_video_folder(folder_path, output_json, model_size="large-v2", hf_token=None):
    ...
    if not folder_path.exists() or not folder_path.is_dir():
        print(f"Folder invalid: {folder_path}")
        return

    transcriber = VideoTranscriberX(model_size=model_size, hf_token=hf_token)
    video_files = sorted([p for ext in ['*.mp4','*.avi','*.mov','*.mkv'] for p in folder_path.rglob(ext)])
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="WhisperX + Pyannote transcription")
    parser.add_argument("folder_path", help="Folder with videos")
